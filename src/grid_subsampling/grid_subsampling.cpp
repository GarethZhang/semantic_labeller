	...
	float sampleDl)
{
	...
	PointXYZ minCorner = min_point(original_points);
	PointXYZ maxCorner = max_point(original_points);
	PointXYZ originCorner = floor(minCorner * (1 / sampleDl) - PointXYZ(1, 1, 1)) * sampleDl;
	...
	unordered_map<size_t, SampledPts> samples;
	samples.reserve(original_points.size());
	...
	for (auto& p : original_points)
	{
		...
		// Fill the sample map
		if (samples.count(mapIdx) < 1)
			samples.emplace(mapIdx, SampledPts(p));
		else
			samples[mapIdx].update_points(p);
	}

	// Get barycenters and closest point indices
	// *****************************************
